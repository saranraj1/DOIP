...
//
// Fail-soft contract (matches §18.8 disconnect pattern):
...
export class WsSimSource implements SimSource {
  ...
  start({ seed, scenarioId }: { seed: number; scenarioId: string }): void {
    ...
    const url =
      ...
      `?seed=${seed}&scenarioId=${encodeURIComponent(scenarioId)}&speed=1`;

    // timeout guard: if WS doesn’t open within 2 s, fall back
    ...
    this.ws.onmessage = (msg) => {
      try {
        const data = JSON.parse(msg.data) as
          ...
          | { type: "log_replay"; events: SimEvent[] };
  ...
  stop() { this.send({ cmd: "stop" }); this.ws?.close(); }
  setSpeed(speed: number) { this.send({ cmd: "speed", value: speed }); }
