...
const ROLES: Array<{ role: Role; name: string; blurb: string }> = [
  { role: "admin", name: "Col. A. Verma", blurb: "Full access · users, audit log, scenarios" },
  {
    role: "planner",
    name: "Maj. R. Sethi",
    blurb: "Mission planning, run control, acknowledgements",
  },
...
const CAPABILITIES = [
  ...
  },
  {
    ...
    blurb:
      "Describe a hypothetical, review the AI-drafted actions, approve, and watch the branch play out.",
  },
  {
    icon: RouteIcon,
    code: "S5",
    ...
    blurb:
      "Waypoint routes with terrain, threat and weather cost breakdowns behind a feasibility gate.",
  },
  {
    icon: Users,
    code: "S14",
    name: "War-room",
    blurb:
      "Shared tactical picture with task board, decision log and per-sector scoping for staff.",
  },
];
      ...
      <main className="mx-auto grid max-w-6xl gap-10 px-4 py-10 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-sm leading-relaxed text-muted-foreground">
            DOIP is a command center for military training and simulation. A deterministic engine
            drives a synthetic battlespace — patrols, convoys, UAVs, weather, incidents — while AI
            handles the paperwork: situation reports with citations, scenario drafts behind an
            approve gate, and what-if branches you can watch play out. Because the world is an event
            log, every run can be replayed, compared and audited down to the tick.
          </p>
